...
(define (make-zero-crossings 
  ...
  (let ((avpt
         ;; last value is an average here in the original version.
         ;; We need it to be an actual stream val
         (/ (+ (stream-car input-stream) 
               last-value) 
            2)))
    (cons-stream
     ;; We care about the sign change when the sign of the averages changes
