...
(define (compile exp target linkage compile-time-env)
  (cond ((self-evaluating? exp)
        ...
        ((let? exp)
         (compile (let->combination exp)  target linkage compile-time-env))
...
(define (compile-lambda-body exp proc-entry c-env)
  (let*
      ((formals (lambda-parameters exp))
        (updated-env (extend-c-env formals c-env)))
    (append-instruction-sequences
     (make-instruction-sequence '(env proc argl) '(env)
      `(,proc-entry
        ...
        (assign env
                ...
                (reg argl)
                (reg env))))
     ;; No longer compile sequence since this is going to use a let
     (compile (scan-out-defines (lambda-body exp)) ;; new
                       'val 'return updated-env))))
...
;; We need lets for this, let's check our new let dispatch
(compile '(let ((a 2)) a) 'val 'next  (setup-c-env))
;((env)
...
;  after-call5)) ;; Passes the eval-by-eyeballing test
...
(compile  EXP-TO-COMPILE 'val 'next (setup-c-env))
;; This is a long one, too difficult to test at the moment. However looks fine,
;;  I guess, does lexical address lookups for 0 and 1, uses lookup for x and y,
;;  extends the environment for even? and odd?, creates an arglist with 2
;;  *unassigned*'s, does two lexical address set.