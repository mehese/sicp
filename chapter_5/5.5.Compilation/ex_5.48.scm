...
(define (compile-and-run expression)
  (let ((instructions
         (assemble (statements
                    ;; That cadr is there because I wanted to keep the quote
                    ;; from the text in the syntax, and since I'm not evaluating
                    ;; the argument in the code itself I need to extract the
                    ;; quoted text
                    (compile (cadr expression) 'val 'return))
                   ;; without the cadr the structure of a call would be
                   ;;  (compile-and-run (define a 5))
                   eceval)))
    instructions))
