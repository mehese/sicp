...
(define (special-procedure? exp compile-time-env)
  (and (pair? exp)
       (assoc (car exp) AVAILABLE-PRIMITVES-PROCEDURES)
       ;; We also compile it only if it hasn't been defined in the compile
       ;;  time env before
...
(define (compile exp target linkage compile-time-env)
  (display (list 'compiling exp compile-time-env)) (newline)
  (cond ((self-evaluating? exp)
        ...
        ((special-procedure? exp compile-time-env)
         (compile-special-procedure exp target linkage compile-time-env))
        ((application? exp)
     ...
     (make-instruction-sequence '(env proc argl) '(env)
      `(,proc-entry
        ...
        (assign env
                ...
                (reg argl)
                (reg env))))
     (compile-sequence (lambda-body exp) 'val 'return updated-env)))) ;; changed
...
(define (spread-arguments argument-list c-env)
  (let
      ((a1 (compile (car argument-list) 'arg1 'next c-env))
       (a2 (compile (cadr argument-list) 'arg2 'next c-env)))
    (preserving
     '(env)
     a1
...
(define (compile-special-procedure exp target linkage c-env)
  (display "compiling special ") (display exp) (newline)
  (let*
      ((ze-operator (car exp))
    ...
    (if (not (eq? linkage 'return))
        ...
        (append-instruction-sequences
         (spread-arguments ze-operands c-env)
...
(compile '(+ 1 2) 'val 'next (setup-c-env)) ;; open codes ✔
