...
;; Modify to handle the case of compound (interpreted) procedures
(set! compile-procedure-call
      (lambda (target linkage)
        (let ((primitive-branch (make-label 'primitive-branch))
          ...
          (let ((compiled-linkage
            ...
            ;; add a check for intepreted procedure
               ...
               (end-with-linkage
                ...
                (make-instruction-sequence
                 ...
                 `((assign ,target
                           ...
                           (reg proc)
                           (reg argl)))))
               ;; Extra logic for compounds below
               compound-branch
               (compile-compound target compiled-linkage)))
             after-call)))))

;; Modified compile-proc-appl from compiler-support.scm, because "Be sure to
;; handle all the same target and linkage combinations as in compile-proc-appl"
(define (compile-compound target linkage)
  (cond ((and (eq? target 'val) (not (eq? linkage 'return)))
         (make-instruction-sequence '(proc) all-regs
           `((assign continue (label ,linkage))
             (save continue) ;; Without this save it will try to pop an empty stack
             (goto (reg compapp))
             )))
        ...
        ((and (eq? target 'val) (eq? linkage 'return))
         (make-instruction-sequence '(proc continue) all-regs
          '((save continue) ;; Without this save it will try to pop an empty stack
            ;(perform (op user-print) (const "In compound apply"))
            (goto (reg compapp))
            )))
...
; works for (compile-and-go '(define (f n) (+ (g n) 0.5))) too