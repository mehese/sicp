...
;; To avoid calling assemble with eceval from eceva 
(define (assembly-instructions exp-statements)
  (assemble exp-statements eceval))

(define (compilation-instruction? expression)
  (tagged-list? expression 'compile-and-run))

(define (compilation-expression expression)
  (cadr expression))
...
(define eceval-operations
  (list
   ...
   (list 'assembly-instructions assembly-instructions)
   (list 'statements statements)
   (list 'compile compile)
   ;; Last problem
   (list 'compilation-instruction? compilation-instruction?)
   (list 'compile-and-run compile-and-run)
   (list 'compilation-expression compilation-expression)
...
read-compile-execute-print loop  
  (perform (op initialize-stack))
  (perform
   (op prompt-for-input) (const ";;; EC-Eval (compiled) input:"))
  (assign exp (op read))
  (assign env (op get-global-environment))
  (assign continue (label print-result))
  ...
  (assign val (op assembly-instructions) (reg exp))
...
eval-dispatch
  ...
  (test (op compilation-instruction?) (reg exp))
  (branch (label compile-and-run-apply))
...
;; NEW
compile-and-run-apply
  (assign exp (op compilation-expression) (reg exp))
  ;(perform (op user-print) (reg exp))
...
;
;(total-pushes = 56 maximum-depth = 17)
;;;; EC-Eval value:
;240  ✔