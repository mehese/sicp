...
eval-dispatch
  ...
  (branch (label ev-application-sym))
...
;; Special implementation for sym
ev-application-sym
  (save continue)
...
;; 1. Special case when combination starts with symbol
;;
;;  (f 2 3) with optimization
;;  (total-pushes = 15 maximum-depth = 5)
;;
;;  (f 2 3) without optimization
;;  (total-pushes = 19 maximum-depth = 5)

;; 2. Most likely not, since a lot of optimization can be obtained by
;;    seeing the *whole* program a priori.
