      ...
      (cond ((eq? message 'get) contents)
            ((eq? message 'set)
             (if tracing-enabled
                 ...
                 (lambda (value) 
                   (display (list "Changing contents of register"
                                  name
                                  "from"
                                  contents
                                  "to"
            ...
            ((eq? message 'trace-off)
             (set! tracing-enabled #f))
            ((eq? message 'trace-on)
             (set! tracing-enabled #t))
            (else
    ...
    (let ((the-ops
      ...
      (define (dispatch message)
        (cond ((eq? message 'start)
              ...
              ((eq? message 'set-trace-register)
               (lambda (reg-name)
                 (let
                     ((r (lookup-register reg-name)))
                   (r 'trace-on)
                   'done)))
              ((eq? message 'unset-trace-register)
               (lambda (reg-name)
                 (let
                     ((r (lookup-register reg-name)))
                   (r 'trace-off)
...
(begin
  ...
  (display "Trace on") (newline)
  ((fact-machine 'set-trace-register) 'val)
  (set-register-contents! fact-machine 'n 4)
  (start fact-machine)
  (display
   (get-register-contents fact-machine 'val)) ;; = 24 ✔
  (newline)
  (newline)
  (newline)
  (display "Trace off") (newline)
  ((fact-machine 'unset-trace-register) 'val)
  (set-register-contents! fact-machine 'n 4)
  (display
   (get-register-contents fact-machine 'val)) (newline)
  (start fact-machine)
  (display
   (get-register-contents fact-machine 'val)) ;; = 24 ✔
  (newline)
