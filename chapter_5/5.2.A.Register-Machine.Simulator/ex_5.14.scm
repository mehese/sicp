...
(begin
  ...
  (start fact-machine)
  (display
   (get-register-contents fact-machine 'val)) ;; = 479001600 ✔
  (fact-machine 'stats)
  (fact-machine 'initialize-stack)
  (fact-machine 'stats)
  (newline)
  (newline)
  (newline)
...
(define (iter nums)
  ;; Loop through different numbers to get the factorial
  (if (null? nums)
      'done-iterating
      (begin
        (let
          ...
          (fact-machine 'initialize-stack)
          (set-register-contents! fact-machine 'n current)
          (start fact-machine)
          (display
...
;     0           5          10          15          20         25          30
;
;; Checks out with larger numbers ✔
