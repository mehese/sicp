...
;; Exercise 5.9: The treatment of machine operations above permits them
...
(define (operation-exp-operands operation-exp)
  ;; Modified here
  (let
      ((operands (cdr operation-exp)))
    (for-each
     (lambda (operand)
       (if (label-exp? operand)
           (error "Can't have label operand here" operation-exp)))
     operands)
  operands))
...
(define toy-machine
  (make-machine
   ...
   ;; Code
   '(controller
     (assign thing (const 1.0))

     code-loop
     (perform (op print) (reg thing))
     (perform (op print) (const 2.0))

     ;; When uncommenting below we get an error as expected ✔
     ;;(perform (op print) (label code-loop))

     (perform (op print) (const "Printed all the things"))
