  ...
  (let ((contents '*unassigned*))
    (define (dispatch message)
      (cond ((eq? message 'get) contents)
            ...
            ((eq? message 'name) name) ;; extra functionality
            (else
...
(define (make-save inst machine stack pc)
  (let ((reg (get-register machine
    ...
    (lambda ()
      ;; Now we push the name of the register to the stack too
      (push stack (cons (get-name reg) (get-contents reg)))
      (advance-pc pc))))
...
(define (make-restore inst machine stack pc)
  (let ((reg (get-register machine
                           (stack-inst-reg-name inst))))
    (lambda ()
      ;; We retrieve the name of the register from the stack and
      ;;  compare it to the name of the stack we save to. If they're
      ;;  not the same we throw an error
      (let*
          ((reg-name       (get-name reg))
           (stack-contents (pop stack))
           (save-name      (car stack-contents))
           (save-value     (cdr stack-contents)))
        (if (eq? reg-name save-name)
            (set-contents! reg save-value)
            (error "Trying to save the contents of the stack to another reg"
                   (list reg-name save-name)))
        (advance-pc pc)))))
...
(define fib-machine
  (make-machine
   ...
   '(controller
     ...
     ;; This new line should now dump an error
     ;(restore n) ;; Errors as expected ✔
     (restore continue)
