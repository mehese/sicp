...
;; This will now contain a stack as well
(define (make-register name)
  (let 
      ((current-val '*unassigned*)
       (stack       (make-stack)))
    (define (dispatch message)
      (cond ((eq? message 'get) current-val)
            ((eq? message 'set)
             (lambda (new-value) (set! current-val new-value)))
            ((eq? message 'initialize-stack)
             (stack 'initialize))
            ((eq? message 'restore)
             (set! current-val (pop stack)))
            ((eq? message 'save)
