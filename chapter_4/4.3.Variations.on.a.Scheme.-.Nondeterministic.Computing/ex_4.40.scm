...
;; See the updated multiple dwelling. Instead of using distinct? checks for everything
;;  we just add the constraint that the currently considered tenant must not be placed
;;  in any of the pre-occupied floors. A few extra lines of code for a massive decrease
;;  in the size of our search space.

'(define (require p)
   (if (not p) (amb)))

'(define (multiple-dwelling-bad)
