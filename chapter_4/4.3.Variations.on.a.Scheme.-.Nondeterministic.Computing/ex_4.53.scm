...
;; Needed to add prime? and prime-sum-pair again. Added prime? as a primitive
;;  while prime-sum-pair is just loaded up with before we start the driver loop
...
(define (analyze-if-fail exp)
  (let ((pproc (analyze (if-predicate exp)))
    ...
    (lambda (env succeed fail)
      (pproc env
             ;; Pass that success forward
             succeed
             ;; Initially forgot to wrap in a lambda, kept getting called
...
(ambeval '(begin
            ...
            (define (prime-sum-pair list1 list2)
              (let ((a (an-element-of list1))
                ...
                (list a b))))
         the-global-environment
         (lambda (val next-alternative) 'loaded-up-some-functions)
         (lambda () '()))
