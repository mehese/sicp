...
(define (make-apply fun argument)
  (list fun argument))
        ...
        (if (cond-else-clause? first)
            (if (null? rest)
                ...
                (error "ELSE clause isn't 
                        last: COND->IF"
                       clauses))
            ;; Check if it's this new special form
            (if (eq? '=> (car (cond-actions first)))
                (let ((predicate (cond-predicate first))
                      (fun       (cadr (cond-actions first))))
                  (make-if predicate
                           (make-apply fun predicate)
                           (expand-clauses rest)))
...
(define my-exp '(cond ((assoc 'b '((a 1) (b 2))) => cadr)
      (else false)))
