...
(define (scan-frame frame var action-if-found action-if-empty)
  (cond ((empty-frame? frame) (action-if-empty))
        ((eq? var (first-variable frame)) (action-if-found frame))
        (else (scan-frame (rest-of-frame frame) var action-if-found action-if-empty))))
  ...
  (define (env-loop env)
    (if (eq? env the-empty-environment)
        ...
        (let ((frame (first-frame env)))
          (scan-frame frame
                      var
                      (lambda (f) (first-value f)) ;; action-if-found 
                      (lambda () (env-loop (enclosing-environment env))))))) ;; action-if-empty
  (env-loop env))
  ...
  (define (env-loop env)
    (if (eq? env the-empty-environment)
        ...
        (let ((frame (first-frame env)))
          (scan-frame frame
                      var
                      (lambda (f) (set-first-value-of-frame f val)) ;; action-if-found
                      (lambda () (env-loop (enclosing-environment env))))))) ;; action-if-empty
  (env-loop env))
...
(define (define-variable! var val env)
  (let ((frame (first-frame env)))
    (scan-frame frame
                var
                (lambda (f) (set-first-value-of-frame f val)) ;; action-if-found
                (lambda () (add-binding-to-frame! var val f))))) ;; action-if-empty
...
(define f (make-frame '(a b c) '(1 2 false)))
(define e (list f))
