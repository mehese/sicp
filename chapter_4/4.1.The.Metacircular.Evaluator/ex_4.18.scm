...
;Consider an alternative strategy for scanning out definitions that translates the example
; in the text to
;
;(lambda <vars>
;  (let ((u '*unassigned*)
...
; not appear in the user’s program. Consider the solve procedure from 3.5.4:
;
...
;    <e3>))
;
;; will fail as it will tr to evaluate (stream-map f y) while y
;; still has the value *unassigned*
...
;    <e3>))
;
;; will work as by the time we are trying to interpret the stream-map instruction
;;  y should already have a value different from *unassigned*
