...
;; Transformed the letrec would look like
;(define (f x)
;  (let
;      ((even? (quote *undefined*))
;       (odd?  (quote *undefined*)))
;    (set! even?
;          (lambda (n)
;            (if (= n 0)
...
;    <rest of body>))
;
;; See ex_4.20.png for environment diagram and explanation on why we cannot substitute
;;  this with a simple let
...
(define (x-undefineds x)
  (define (iter num lst)
    (if (= 0 num)
        lst
        (iter (dec num) (cons ''*undefined* lst))))
  (iter x '()))
...
(define (letrec->combination exp)
  (let
      ((vars (let-variables exp))
       (exps (let-expressions exp))
       (body (let-body exp)))

    (make-multiple-let
     vars
     (x-undefineds (length vars))
     (append
      (map make-assignment vars exps)
      body))))
