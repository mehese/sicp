...
(define (eval exp env)
  (cond ((self-evaluating? exp) 
        ...
        ;; We moved this one up here
        ((application? exp)
         (display "I am an application ") (newline)
         (display "My operator is") (display (operator exp)) (newline)
         (display "My operands are") (display (operands exp))
...
; remember that this is how `application?` works
;  (define (application? exp) (pair? exp)

; *if* we had a working evaluator (define x 3) would throw an error as the
;  evaluator would try to apply define as a procedure, which would mean trying to
;  evaluate x which would result in an error.
...
;; In the shell if we run 
;    (eval '(call + 5 5) '())
;  the output will be
; I am an application 
; My operator is+
; My operands are(5 5)bump

;; And if we run
;    (eval '(define x 5) '())
;  the output will be I am a definition ok
