...
;; The new version cracks when having to run
;(define f1 (lambda (x) (+ x x)))

;(define f2 (lambda (f x) (f x)))

;(f2 f1 5)

;; Unknown procedure type: APPLY {thunk f1 [...]

;; At the definition phase f1 is basically transformed into a thunk.
;;  Calling eval rather than actual value on a thunk tries to apply 'thunk of arguments
;;  which doesn't work. Calling it with actual-value works, as actual value accepts both
;;  thunks and non-thunks
...
(define (eval exp env)
  (cond ((self-evaluating? exp) 
        ...
        ((application? exp)
         (apply-l (eval (operator exp) env) ;; (actual-value (operator exp) env)
                  (operands exp)
                env))        
        (else
