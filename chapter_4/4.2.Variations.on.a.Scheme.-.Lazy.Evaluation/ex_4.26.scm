...
;; Unless is effectively an alias for if, why should we have an extra special form for the
;;  effectively the same thing?

;; See line 170 for special implementation. Also makes the previous problem's factorial
;;  implementation work. Maybe it's useful in a code where one would need to do a lot of
;;  exception checking, and they would like to avoid (if (not (cond)) ...) type structures.
...
(define (analyze-unless exp)
  (let ((pproc (analyze (if-predicate exp)))
        (good-proc (analyze (if-consequent exp)))
        (bad-proc (analyze (if-alternative exp))))
    (lambda (env)
      (if (true? (pproc env))
          (bad-proc  env)
          (good-proc env)))))
