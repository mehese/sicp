...
; Rather than bother with explicitly tagging lazy lists, we're just checking if the
; it's a lambda with parameter m with the body of cons (i.e. (m x y). The only drawback
; is that if the user decides to create a different function that has the body of cons,
; it will display it as a lazy list.

; Got "inspired" from here
;  http://lawfulsamurai.blogspot.com/2009/01/sicp-section-42-variations-on-scheme.html

; I modified the display function to work with better with nested lists. It doesn't do
;  the neat thing Racket manages to do where it detects that detects cycles in lists

;; Here's how a few test cases look like

;;;; L-Eval input:
;(cons (cons (cons (cons 1 2) 'None) 'None)
;'None)
;
;;;; L-Eval value:
;(((...) None) None)
...
;; Modified this to deal with nested lists
(define (display-lazy-list obj)
  (define MAX-LENGTH 3)
  (define MAX-DEPTH 2)
  (define (display-lazy-pair exp length-index depth-index)
    (define (get-actual-value op exp)
      (actual-value (list op exp) the-global-environment))  
    (cond
      ((>= length-index MAX-LENGTH) '(...)) ;; Escape lengthwise
      ((>= depth-index MAX-DEPTH) '(...)) ;; Escape high nestedness (for trees)
      (else (let ((first (get-actual-value 'car exp))
              (rest (get-actual-value 'cdr exp)))
          (append
           (if (lazy-list? first)
               (list (display-lazy-pair (list 'car exp)
                                        (+ length-index 1)
                                        (+ depth-index 1)))
               (list first))
           (if (lazy-list? rest)
               (display-lazy-pair (list 'cdr exp)
                                  (+ length-index 1)
                                  depth-index) ;; cdr doesn't descend in depth
               (list rest)))))))
  (display (display-lazy-pair obj 0 0)))
