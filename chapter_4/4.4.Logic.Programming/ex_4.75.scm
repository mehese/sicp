...
;; See implementation on uniquely asserted. Teste
...
'(and (job ?person-x ?job)
      (unique (supervisor  ?other ?person-x)))
...
(define (uniquely-asserted query-pattern frame-stream)
  ...
  (stream-flatmap
   ...
   (lambda (frame)
     (let
         ;; it should use qeval to find the stream of all extensions to the frame
         ;; that satisfy the given query
         ((res-stream (qeval (car query-pattern) (singleton-stream frame))))
       ;;  Any stream that does not have exactly one item in it should be eliminated.
       (if (is-unit-stream? res-stream)
           res-stream
           the-empty-stream)))
   frame-stream))
