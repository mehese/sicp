...
;; And it ends up in an infinite loop. WHy did swapping the outranked-by and supervisor
;;  constraints cause this? In the previous case outranked by was called on an already
;;  constrained ?middle-manager in the end. In this case ?middle-manager is not constrained
;;  so effectively it's the same outranked-by query is run in a loop, as no new constraints
;;  are applied on the variables in between calls.
