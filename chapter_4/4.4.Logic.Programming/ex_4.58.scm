...
(define custom-rules
  '(          
    ...
    (rule (big-shot ?person)
          (and
           (job ?person (?division . ?etc))
           (or (and (supervisor ?person ?supervisor) ;; has a supervisor
                    (job ?supervisor (?super-division . ?etc2)) 
                    (not (same ?division ?super-division))) ;; doesn't work
               (not  (supervisor ?person ?supervisor))) ;; doesn't hava a supervisor
                                                        ;;  i.e. (Warbucks Oliver)
           ))
    ))
