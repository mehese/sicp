...
;; We just take the car of all non-null streams since all non-null streams
;;  will be singletons
'(define (simple-flatten stream)
   (stream-map stream-car
               (stream-filter (lambda (substream) (not (stream-null? substream))) 
                              stream)))
...
'(and (salary ?person ?pay) (lisp-value <= ?pay 35000))
...
(define (simple-flatten stream)
  (stream-map stream-car
              (stream-filter
               (lambda (substream) (not (stream-null? substream))) 
               stream)))
