...
;; As the stream is self-referential, the minute we add an assertion with
;;  the new syntax it becomes an infinite stream of just one value

;; So the query

'(display-stream THE-ASSERTIONS)

;; Shows everything just fine. But if we type in

'(add-assertion! '(married Minnie Mickey))

;; then by trying to rerun the display-stream instruction we realize
;;  that now the whole stream is an infinite loop that displays
;;  just (married Minnie Mickey)
