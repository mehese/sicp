...
(#%require (only racket/base eqv-hash-code))

;; Indeed typing in Alyssa's query returns each pair twice
...
;; Not much of a way of going about this unless we can impose some sort of
;;  sorting  on the users in the db, and then impose the condition that
;;  (lisp-value < (sorting ?person-1) (sorting ?person-2)). See the
;;  eqv-hash-code we imported above. Now if we redefine lives-near to

'(rule (lives-near ?person-1 ?person-2)
      (and (address ?person-1 (?town . ?rest-1))
           (address ?person-2 (?town . ?rest-2))
           (lisp-value smaller-hash ?person-1 ?person-2)
...
;; Observe that 'smaller-hash is defined on the car of the name
;;  (lambda (x y) (< (eqv-hash-code (car x)) (eqv-hash-code (car y))))
;; This is because eqv-hash-code does not return the same number on the same
;; list.
...
(define primitive-procedures
  (list (list 'car car)
        ...
        (list '>= >=)
        (list '> >)
        (list 'eqv-hash-code eqv-hash-code)
        (list 'smaller-hash (lambda (x y)
                                (< (eqv-hash-code (car x)) (eqv-hash-code (car y)))))
        ))
