...
(define custom-rules
  '(
    ...
    ;; New rules
    (rule (is-a-grandson (grandson)))
    (rule (is-a-grandson (great . ?rest)) ;; Don't forget the parens around great . ?rest
          (is-a-grandson ?rest))

    ;; Eventually ?rel will take the value (grandson)
    (rule ((grandson) ?x ?y) 
          (grandson ?x ?y))
...
'((great great great grandson) ?x ?y)
