...
(define custom-rules
  '(
    ...
    ;; Base cases (uncommenting the other base cases results in more results returned
    ;(rule (reverse () ()))
    (rule (reverse (?x) (?x)))
    ;(rule (reverse (?x ?y) (?y ?x)))

    ;; General case
    (rule (reverse (?xhead . ?xtail) ?y)
          (and (reverse ?xtail ?reverse-xtail)
               (append-to-form ?reverse-xtail (?xhead) ?y)))
    ))
...
;;; Query results: INFINITE LOOP T_T
