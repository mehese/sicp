...
;; A first issue is how do we consider if a real can be transformed to a rational? Computers can't
;;  handle irrational numbers, so what do we do?  ¯\_(ツ)_/¯ 
...
;; rememeber this one is poorly defined
(define (real->rational re)
  (let
      ((rat-candidate (valid-rational (contents re))))
    (if rat-candidate
        (make-rational (car rat-candidate) (cdr rat-candidate))
        #f)))

;; For the purpose of the problem, go through all p and qs from
;;  1->9, and if any of them equal our number within tol, just
;;  accept it as a valid rational. Otherwise pretend it's irrational
(define (valid-rational x)
  (define tol 1e-6)
  (define (within-tol number)
    (< (abs (- x number))
       tol))
  ;; this loop does a lot of uneccessary work, since 3/2 <=> 6/4
  (define (check-fraction p q)
    (let
        ((fraction (/ p q)))
      (cond
        ((within-tol fraction) (cons (inexact->exact p) q))
        ((= q 10) #f)
        ((= p 10) (check-fraction 1 (inc q)))
        (else (check-fraction (inc p) q)))))
  (if (integer? x)
      (cons (inexact->exact x) 1) 
      (check-fraction 1 2)))


(define (rational->scheme ra)
  (let
      ((p (numer ra))
       (q (denom ra)))
    (cond
      ((= 1 q) (make-scheme-number p))
      ((= q (gcd p q)) (make-scheme-number (/ p q)))
      (else #f))))
...
(put-coercion 'project 'complex complex->real)
(put-coercion 'project 'real real->rational)
(put-coercion 'project 'rational rational->scheme)

(define (project number)
  (let
      ((projection (get-coercion 'project (type-tag number))))
    (if projection
        (projection number)
        #f)))
...
;; Projects a number as low as possible
(define (drop number)
  (if (project number)
      (drop (project number))
      number))
...
(define (apply-generic op . args)
  (let ((type-tags (map type-tag args)))
    (let ((proc (get op type-tags)))
      (if proc
          (if (= (length args) 1)
              ;; without this real-part, imag-part and others will break
...
(drop i); ✔
(define pi-complex (make-complex-from-mag-ang 3.1415 0))
(drop pi-complex); ✔
(define c2 (make-complex-from-real-imag 0.5 0))
(drop c2); ✔
(drop (make-complex-from-mag-ang 1.6666666666666666666666666 0)); ✔


(add s1 ra1 (make-complex-from-real-imag 0 0) (make-rational 1 3)); ✔
(add c2 c2); ✔
(add ra1 s1); ✔
(add (make-real 0.33333333333333) (make-rational 1 3) (make-complex-from-mag-ang 0.333333333333 0)); ✔
(sub (make-rational 3 2) (make-real 0.5)); ✔