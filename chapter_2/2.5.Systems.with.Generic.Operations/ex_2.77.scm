...
;; Alyssa's updates don't work without these declarations, as the program
;;  will default to the operations implemented in the language
...
;; Without the command below the following error is thrown from apply-generic
...
;;    here: type-tags <- '(complex)
;;          proc <- (get 'magnitude '(complex))
;;   If Allyssa's table updates aren't applie we get the error below
;;   otherwise
;;          proc <- (lambda (x) (apply-generic 'magnitude x))
;;          => (apply proc (map contents args))
;;              <=> (apply (lambda (x) (apply-generic 'magnitude x)) ({rectangular 3 . 4}))
;;              = 5
;; apply generic is called twice in total, first extracting a complex tag, second time extracting
;;  the rectangular tag